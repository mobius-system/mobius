...
async function testBundleArchAndUrl() {
  ...
  ok(before.includes('mobius-python-linux-x64-v5') && before.endsWith('.zip'), 'bundleUrl follows the fixed filename pattern')
  const saved = process.env.MOBIUS_TUI_PYTHON_BUNDLE_URL
  process.env.MOBIUS_TUI_PYTHON_BUNDLE_URL = 'https://example.test/cdn/'
  try {
    ok(bundleUrl('win-x64') === 'https://example.test/cdn/mobius-python-win-x64-v5.zip', 'MOBIUS_TUI_PYTHON_BUNDLE_URL overrides the CDN base and trims trailing slash')
