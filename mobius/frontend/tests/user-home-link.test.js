...
const userHomeTarget = buttonMarkup.match(/to=\{(userParam\s*\?\s*homePath\(userParam\)\s*:\s*'\/')\}/)
assert(userHomeTarget, '顶栏用户名按钮必须声明主页导航目标')
assert.equal(
  userHomeTarget[1].replace(/\s+/g, ''),
  "userParam?homePath(userParam):'/'",
  '顶栏用户名按钮必须通过 canonical helper 导航到当前用户主页，并在用户参数缺失时回退根路径',
)
