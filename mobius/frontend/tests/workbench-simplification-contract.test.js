...
const easyPromptTitleRule = easyJsonlCssSource.match(/\.easy-jsonl-prompt__meta strong\s*\{([^}]*)\}/)?.[1] || ''
...
assert.match(easyPromptRule, /background:\s*color-mix\(in srgb,\s*var\(--surface-card\)\s*72%,\s*var\(--surface-raised\)\)/, '用户任务卡必须使用中性的 card / raised surface token')
assert.match(easyPromptRule, /border-radius:\s*12px/, '用户任务卡圆角必须是 12px')
assert.match(easyPromptRule, /box-shadow:\s*inset 0 1px 0/, '用户任务卡必须保留 inset 高光')
assert.match(easyPromptAvatarRule, /border:\s*1px solid var\(--border-default\)[\s\S]*background:\s*var\(--surface-control\)[\s\S]*color:\s*var\(--text-secondary\)/, '用户任务头像必须使用中性边框、表面和图标色')
assert.match(easyPromptAvatarRule, /border-radius:\s*50%/, '用户任务头像必须保持圆形')
assert.match(easyPromptTitleRule, /color:\s*var\(--text-secondary\)/, '用户任务标题必须使用中性文字色')
assert.doesNotMatch(easyPromptTitleRule, /var\(--accent-primary\)/, '用户任务标题不得使用 accent-primary')
assert.match(easyPromptBodyRule, /font-size:\s*14px[\s\S]*line-height:\s*1\.55/, '用户任务正文必须保留 14px / 1.55 排版')
assert.match(easyResponseRule, /border:\s*1px solid var\(--border-default\)/, '模型回复卡必须使用 --border-default')
assert.match(easyResponseRule, /border-radius:\s*12px/, '模型回复卡圆角必须是 12px')
assert.match(easyResponseRule, /padding:\s*16px 18px/, '模型回复卡必须保留紧凑的卡片内边距')
assert.doesNotMatch(easyResponseRule, /margin-left|padding-left/, '模型回复卡不得再通过左侧头像偏移塑形')
