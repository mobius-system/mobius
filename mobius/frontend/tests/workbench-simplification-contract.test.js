...
assert.match(easyJsonlCssSource, /\.easy-jsonl-activity, \.easy-jsonl-live \{[^}]*min-height:\s*24px/, '折叠活动行高度必须压到 24px')
assert.match(easyJsonlCssSource, /\.easy-jsonl-activity__summary \{[\s\S]*padding:\s*2px 2px 2px 22px/, '折叠活动摘要必须使用 Codex 式透明紧凑内边距')
...
assert.match(workPageSource, /hidden=\{!activeSessionLoaded\}[\s\S]*className=\{`workbench-session-chat__surface \$\{activeSessionLoaded \? 'flex' : 'hidden'\}/, '切换 Session 时保活的旧 Chat 必须使用专用 surface 和显式 display 类退出加载期布局')
...
assert.match(railSource, /const toggleFolder = \(folder: ProjectFolder\) => \{[\s\S]{0,180}if \(normalizedQuery\) return\s*const folderKey/, '除搜索结果外，项目文件夹必须始终响应用户的折叠操作')
