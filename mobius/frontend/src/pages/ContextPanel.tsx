      ...
      <div className="fixed top-0 right-0 h-full w-[28rem] z-50 flex flex-col shadow-2xl border-l" style={{ background: bgPrimary, borderColor }}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[length:var(--fs-xl)] font-semibold" style={{ color: textPrimary }}>会话上下文快照</h2>
              <p className="text-[length:var(--fs-xs)]" style={{ color: textMuted }}>只读 · 在新建会话前定型</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {/* 用户 */}
          <section>
            <h3 className="flex items-center gap-2 text-[length:var(--fs-md)] font-semibold mb-2" style={{ color: textPrimary }}>
            ...
            <div className="rounded-lg p-3 space-y-1.5" style={{ background: bgSecondary, border: `1px solid ${borderColor}` }}>
              <div className="flex justify-between text-[length:var(--fs-sm)]">
                <span style={{ color: textMuted }}>姓名</span>
                <span style={{ color: textPrimary }}>{user?.display_name || '-'}</span>
              </div>
              <div className="flex justify-between text-[length:var(--fs-sm)]">
                <span style={{ color: textMuted }}>角色</span>
                <span className="px-1.5 py-0.5 rounded text-[length:var(--fs-xs)]" style={{ background: isDark ? 'rgba(59,130,246,0.15)' : 'rgba(59,130,246,0.12)', color: isDark ? '#60a5fa' : '#2563eb' }}>
              ...
              <div className="rounded-lg p-3 space-y-1.5" style={{ background: bgSecondary, border: `1px solid ${borderColor}` }}>
                ...
                {currentProject.description && (
                  <div className="pt-1.5 border-t" style={{ borderColor }}>
                    <p className="text-[length:var(--fs-xs)] leading-relaxed" style={{ color: textMuted }}>{currentProject.description}</p>
                  </div>
                )}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="rounded-lg p-3 space-y-1.5" style={{ background: bgSecondary, border: `1px solid ${borderColor}` }}>
                ...
                {currentIssue.description && (
                  <div className="pt-1.5 border-t" style={{ borderColor }}>
                    <p className="text-[length:var(--fs-xs)] leading-relaxed" style={{ color: textMuted }}>{currentIssue.description}</p>
                  </div>
                )}
              </div>
            </section>
          ...
          {currentSession && (
            <section>
              ...
              <div className="rounded-lg p-3 space-y-1.5" style={{ background: bgSecondary, border: `1px solid ${borderColor}` }}>
                {skills.length === 0 && <p className="text-[length:var(--fs-xs)] italic" style={{ color: textMuted }}>本会话未注入任何 Skill</p>}
                {skills.map(sk => (
                  (() => { const share = injectionShare(sk, fullTextLength); return (
                  <div key={sk.id} className="flex items-start justify-between gap-2 text-[length:var(--fs-sm)]">
                    <div className="min-w-0 flex-1">
                      <div style={{ color: textPrimary }} className="truncate">{sk.name}</div>
                      {sk.description && <div className="text-[length:var(--fs-xs)] truncate" style={{ color: textMuted }}>{sk.description}</div>}
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      <span className="px-1.5 py-0.5 rounded text-[length:var(--fs-xs)]" style={{ background: isDark ? 'rgba(168,85,247,0.15)' : 'rgba(168,85,247,0.1)', color: isDark ? '#c084fc' : '#7e22ce' }}>{SCOPE_LABEL[sk.scope] || sk.scope}</span>
                      <span title={`${share.length} 字，占完整注入文本 ${share.percent}%`} className="px-1.5 py-0.5 rounded text-[length:var(--fs-xs)]" style={{ background: share.highlighted ? (isDark ? 'rgba(234,179,8,0.2)' : 'rgba(234,179,8,0.15)') : (isDark ? 'rgba(34,197,94,0.18)' : 'rgba(34,197,94,0.12)'), color: share.highlighted ? (isDark ? '#fde68a' : '#a16207') : (isDark ? '#86efac' : '#15803d') }}>{share.percent}%</span>
                    </div>
                  </div>
                  ) })()
                ))}
          ...
          {currentSession && (
            <section>
              ...
              <div className="rounded-lg p-3 space-y-1.5" style={{ background: bgSecondary, border: `1px solid ${borderColor}` }}>
                {memories.length === 0 && <p className="text-[length:var(--fs-xs)] italic" style={{ color: textMuted }}>本会话未注入任何 Memory</p>}
                {memories.map(m => (
                  (() => { const share = injectionShare(m, fullTextLength); return (
                  <div key={m.id} className="flex items-start justify-between gap-2 text-[length:var(--fs-sm)]">
                    <div className="min-w-0 flex-1">
                      <div style={{ color: textPrimary }} className="truncate">{m.name}</div>
                      {m.description && <div className="text-[length:var(--fs-xs)] truncate" style={{ color: textMuted }}>{m.description}</div>}
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      <span className="px-1.5 py-0.5 rounded text-[length:var(--fs-xs)]" style={{ background: isDark ? 'rgba(34,197,94,0.15)' : 'rgba(34,197,94,0.1)', color: isDark ? '#86efac' : '#15803d' }}>{SCOPE_LABEL[m.scope] || m.scope}</span>
                      <span title={`${share.length} 字，占完整注入文本 ${share.percent}%`} className="px-1.5 py-0.5 rounded text-[length:var(--fs-xs)]" style={{ background: share.highlighted ? (isDark ? 'rgba(234,179,8,0.2)' : 'rgba(234,179,8,0.15)') : (isDark ? 'rgba(34,197,94,0.18)' : 'rgba(34,197,94,0.12)'), color: share.highlighted ? (isDark ? '#fde68a' : '#a16207') : (isDark ? '#86efac' : '#15803d') }}>{share.percent}%</span>
                    </div>
                  </div>
                  ) })()
            ...
            <section>
              <div className="flex items-center justify-between mb-2">
                <button onClick={() => setShowBody(v => !v)}
                  className="flex-1 flex items-center justify-between text-[length:var(--fs-md)] font-semibold"
                ...
                <button onClick={(e) => {
                  ...
                  className="ml-2 text-[length:var(--fs-xs)] px-2 py-0.5 rounded border transition-colors"
                  style={{ borderColor, color: copied ? '#22c55e' : textMuted }}>
                  {copied ? '✓ 已复制' : '复制'}
                </button>
              </div>
              {showBody && (
                <pre className="rounded-lg p-3 text-[length:var(--fs-xs)] leading-snug whitespace-pre-wrap break-words max-h-96 overflow-y-auto"
        ...
        <div className="px-5 py-3 border-t" style={{ borderColor }}>
          <p className="text-[length:var(--fs-xs)] text-center leading-relaxed" style={{ color: textMuted }}>
