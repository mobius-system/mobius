...
export function homePath(userId: string, options: { projectId?: string; view?: string; panel?: string } = {}) {
  return withQuery(`/u/${segment(userId)}/easy_mode`, {
...
// 极简版专项团队 (多智能体 Research) 页: /easy_mode?research=<id>&project=<pid>
export function easyResearchPath(userId: string, researchId: string, options: { projectId?: string } = {}) {
  return withQuery(`/u/${segment(userId)}/easy_mode`, {
    research: researchId,
    project: options.projectId,
  })
...
// 极简版「专项团队」专属欢迎页: /easy_mode?view=research (项目欢迎页的同构变体,
// 在这里只跟 Leader 对话, 由 Leader 自动组队并推进 Research)。
export function researchHomePath(userId: string, options: { projectId?: string } = {}) {
  return homePath(userId, { view: 'research', projectId: options.projectId })
}

// 极简版「我的创作」专属欢迎页: /easy_mode?view=creation (项目欢迎页的同构变体,
// 项目默认就是拓展项目, 输入内容围绕所选拓展展开, 进会话后右侧实时预览拓展)。
export function creationHomePath(userId: string, options: { projectId?: string } = {}) {
  return homePath(userId, { view: 'creation', projectId: options.projectId })
}
...
export function easyResearchNavigation(
  userId: string,
  researchId: string,
  options: { projectId?: string; returnTo?: string } = {},
) {
  return target(easyResearchPath(userId, researchId, options), 'main-heading', options.returnTo)
}

export function researchHomeNavigation(
  userId: string,
  options: { projectId?: string; returnTo?: string } = {},
) {
  return target(researchHomePath(userId, options), 'composer', options.returnTo)
}

export function creationHomeNavigation(
  userId: string,
  options: { projectId?: string; returnTo?: string } = {},
) {
  return target(creationHomePath(userId, options), 'composer', options.returnTo)
}
...
export function workbenchReturnContext(value: unknown): WorkbenchReturnContext {
  ...
  try {
    ...
    if (url.pathname.endsWith('/easy_mode') && url.searchParams.get('session')) return 'session'
  } catch {}
...
export function legacySessionRedirect(
  ...
  const target = new URL(sessionPath(userId, sessionId), INTERNAL_ORIGIN)
