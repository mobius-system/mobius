...
export function ProjectFileWorkspace({
  ...
  const fileTabsSlot = useWorkbenchShellSlot('file-tabs')
  ...
  if (previewSlot) {
    return (
      <>
        {fileTabsSlot && createPortal(tabs, fileTabsSlot)}
        {createPortal(panel, previewSlot)}
      </>
    )
  }
...
export function FilePreviewLayer({
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        const candidates = projectPathCandidates(resolved.path, projectMeta?.bind_path)
        let file: any = null
        ...
        <div className="code-artifact-preview__chrome">
          ...
          {crumbs.length > 0 && (
            <header className="code-artifact-preview__header">
              <nav className="code-artifact-preview__crumb" aria-label="当前文件路径">
  ...
  if (previewSlot) {
    return (
      <>
        {fileTabsSlot && createPortal(tabsBar, fileTabsSlot)}
        {createPortal(panel, previewSlot)}
      </>
    )
  }
