 ...
 */
export function projectPathCandidates(path: string, bindPath?: string | null): string[] {
  ...
  // Agent output often prefixes a project-relative path with the project
  // directory name (for example `native-easy/services/foo.ts`). The preview
  // resolver already anchors relative paths at bindPath, so that prefix would
  // otherwise be duplicated and produce a false 404. Keep the exact candidate
  // first, then retry without the root basename only when the API reports 404.
  const rootName = normalizeAbsolutePath(String(bindPath || ''))
    .split('/').filter(Boolean).at(-1) || ''
  if (rootName) {
    const rootPrefix = `/${rootName}/`
    if (normalized.startsWith(rootPrefix)) add(`/${normalized.slice(rootPrefix.length)}`)
  }
