...
import { ChevronDown, ChevronLeft, ExternalLink, KeyRound, LogOut, UserRound, X } from 'lucide-react'
...
} from '../../services/easy-workbench/client-distribution'

export type SettingsSection = 'general' | 'account' | 'context' | 'connections' | 'advanced' | 'admin'
...
export function SettingsPanel({
  ...
  const {
    user, theme, setTheme, assistantBubbleEnabled, toggleAssistantBubble, currentProject, logout,
  } = useStore()
    ...
    <div className="settings-panel theme-overlay workbench-layer-modal fixed inset-0 flex items-center justify-center p-3">
      ...
      <div ref={panelRef} role="dialog" aria-modal="true" aria-label="设置"
        ...
        <div className={`${mobileContentOpen ? 'block' : 'hidden'} min-w-0 flex-1 overflow-y-auto p-4 pt-14 sm:block sm:p-5 sm:pr-14`} style={{ background: 'var(--surface-right)' }}>
          ...
          {section === 'account' && (
            <section>
              <h2 ref={sectionHeadingRef} tabIndex={-1} className="text-[16px] font-semibold outline-none" style={{ color: 'var(--text-primary)' }}>账户</h2>
              <p className="mt-1 text-[12px]" style={{ color: 'var(--text-muted)' }}>当前登录的账户信息与安全操作。</p>
              <div className="mt-5 flex max-w-xl items-center gap-3 rounded-[var(--radius-panel)] border p-4" style={{ borderColor: 'var(--border-default)', background: 'var(--surface-card)' }}>
                <span className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full" style={{ background: 'color-mix(in srgb, var(--accent-primary) 14%, transparent)', color: 'var(--accent-primary)' }}>
                  <UserRound className="h-5 w-5" />
                </span>
                <span className="min-w-0">
                  <span className="block truncate text-[13px] font-medium" style={{ color: 'var(--text-primary)' }}>{user?.display_name || user?.id || '未登录'}</span>
                  <span className="mt-0.5 block truncate text-[11px]" style={{ color: 'var(--text-muted)' }}>
                    {user?.id || '—'}{user?.role ? ` · ${user.role === 'admin' ? '管理员' : user.role === 'member' ? '成员' : user.role}` : ''}
                  </span>
                </span>
              </div>
              <div className="mt-4 grid max-w-xl gap-2">
                <button type="button" onClick={() => setModal('password')} className="workbench-control-md flex items-center gap-2 border px-3 text-[12px] hover:bg-[var(--surface-control-hover)]" style={{ borderColor: 'var(--border-default)', color: 'var(--text-secondary)' }}>
                  <KeyRound className="h-3.5 w-3.5 flex-shrink-0" />修改密码
                </button>
                <button type="button" onClick={() => { logout(); navigate('/') }} className="workbench-control-md flex items-center gap-2 border px-3 text-[12px] hover:bg-[var(--status-danger-soft)]" style={{ borderColor: 'var(--border-default)', color: 'var(--status-danger)' }}>
                  <LogOut className="h-3.5 w-3.5 flex-shrink-0" />退出登录
                </button>
              </div>
            </section>
          )}
