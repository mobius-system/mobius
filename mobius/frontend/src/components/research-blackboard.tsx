    ...
    <div className="flex flex-col h-full min-h-0" style={{ background: 'var(--bg-primary)' }} data-tour="research-blackboard">
      <div className="px-3 py-2 border-b flex items-center gap-2 flex-shrink-0" style={{ borderColor: 'var(--border-color)' }}>
        <div className="flex-1 min-w-0">
          <div className="text-[length:var(--fs-md)] font-semibold flex items-center gap-1.5" style={{ color: 'var(--text-primary)' }}>
            <svg className="w-3.5 h-3.5 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
            </svg>
            Blackboard
          </div>
          <div className="text-[length:var(--fs-xs)] mt-0.5 flex items-center gap-1.5" style={{ color: 'var(--text-muted)' }}>
        ...
        <label className="flex items-center gap-1 text-[length:var(--fs-xs)] cursor-pointer" style={{ color: 'var(--text-muted)' }} title="开启后每 4 秒自动刷新">
        ...
        <button onClick={() => fetchBlackboard(false)} title="立即刷新"
          className="h-6 px-2 rounded text-[length:var(--fs-xs)] border transition-colors hover:bg-[var(--bg-hover)]"
      ...
      <div className="px-3 py-2 border-b flex items-center gap-1 flex-shrink-0" style={{ borderColor: 'var(--border-color)' }}>
        {(['all', 'pending', 'delivered'] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className="px-2 h-6 rounded text-[length:var(--fs-xs)] border transition-colors"
            style={{
        ...
        <button onClick={toggleOrder} title={descending ? '当前: 最新在上 (倒序) — 点击切换为正序' : '当前: 最早在上 (正序) — 点击切换为倒序'}
          className="ml-auto px-2 h-6 rounded text-[length:var(--fs-xs)] border transition-colors flex items-center gap-1 hover:bg-[var(--bg-hover)]"
          style={{ borderColor: 'var(--border-color)', color: 'var(--text-secondary)' }}>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        {state.loading && entries.length === 0 ? (
          <div className="flex items-center justify-center h-full text-[length:var(--fs-md)]" style={{ color: 'var(--text-muted)' }}>正在加载...</div>
        ) : state.error ? (
          ...
          <div className="p-2 flex flex-col gap-2">
            {filtered.map((entry, idx) => {
              if (entry.error || !entry.record) {
                return (
                  <div key={`err-${entry.lineNo}`} className="rounded-2xl border p-2 text-[length:var(--fs-sm)]"
                    style={{ background: 'var(--bg-secondary)', borderColor: 'rgba(239,68,68,0.4)', color: '#f87171' }}>
                    <div className="font-mono text-[length:var(--fs-xs)] mb-1">行 {entry.lineNo} · 解析失败: {entry.error}</div>
              ...
              return (
                <div key={key} className="rounded-2xl border overflow-hidden"
                  ...
                  <div className="px-2.5 py-1.5 flex items-center gap-2 border-b" style={{ borderColor: 'var(--border-color)' }}>
                    <div className="flex-1 min-w-0">
                      <div className="text-[length:var(--fs-sm)] font-medium truncate" style={{ color: 'var(--text-primary)' }} title={r.author}>
                        {r.author || 'anonymous'}
                      </div>
                      <div className="text-[length:var(--fs-xs)]" style={{ color: 'var(--text-muted)' }} title={r.created_at}>
                        {r.created_at ? timeAgoPrecise(r.created_at) : '—'}
                      </div>
                    </div>
                    <span className={`px-1.5 py-0.5 rounded border text-[length:var(--fs-2xs)] leading-none ${badge.cls}`}>{badge.label}</span>
                  </div>
                  <div className="px-2.5 py-2 text-[length:var(--fs-sm)] leading-relaxed prose-graph break-words" style={{ color: 'var(--text-secondary)' }}>
                    <ReactMarkdown
                      remarkPlugins={MARKDOWN_REMARK_PLUGINS}
                      rehypePlugins={MARKDOWN_REHYPE_PLUGINS}
                      components={{ a: ({ href, children }) => <a href={href} target="_blank" rel="noreferrer">{children}</a>, pre: MarkdownPre as any }}
                    >
                      {r.content || '_(无正文)_'}
                    </ReactMarkdown>
                  </div>
                  {(targetIds.length > 0 || hasMeta || r.delivery?.last_error) && (
                    <button onClick={() => toggleExpand(key)}
                      className="w-full px-2.5 py-1 text-[length:var(--fs-xs)] flex items-center justify-between border-t hover:bg-[var(--bg-hover)] transition-colors"
                  ...
                  {isOpen && (
                    <div className="px-2.5 py-2 border-t text-[length:var(--fs-xs)] flex flex-col gap-1.5" style={{ borderColor: 'var(--border-color)', color: 'var(--text-muted)' }}>
                      {targetIds.length > 0 && (
                        <div>
                      ...
                      {hasMeta && (
                        <div>
                          ...
                          <pre className="font-mono text-[length:var(--fs-xs)] whitespace-pre-wrap break-all p-1.5 rounded"
      ...
      {state.lastUpdated && (
        <div className="px-3 py-1.5 border-t text-[length:var(--fs-xs)] flex-shrink-0 flex items-center justify-between"
          style={{ borderColor: 'var(--border-color)', color: 'var(--text-muted)' }}>
