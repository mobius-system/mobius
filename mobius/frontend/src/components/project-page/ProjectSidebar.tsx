  ...
  return (
    <>
      {/* <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: 'var(--border-color)' }}>
        <span className="text-[length:var(--fs-md)] font-semibold" style={{ color: 'var(--text-muted)' }}>任务</span>
      ...
      <div className="px-3 py-2 space-y-2">
        <div className="relative">
          ...
          <input value={search} onChange={e => onSearchChange(e.target.value)}
            ...
            placeholder="搜索任务或会话..."
            className="w-full h-8 pl-8 pr-8 rounded-lg text-[length:var(--fs-md)] focus:outline-none focus:border-blue-500/30"
        ...
        <div className="flex gap-1">
          {(['active', 'all', 'completed'] as const).map(f => (
            <button key={f} onClick={() => onFilterChange(f)}
              className={`flex-1 h-7 rounded text-[length:var(--fs-sm)] transition-colors ${
        ...
        {search.trim() && (sessionSearchError || sessionSearchTruncated) && (
          <div className="px-0.5 text-[length:var(--fs-xs)] leading-4" style={{ color: sessionSearchError ? '#f59e0b' : 'var(--text-muted)' }}>
      ...
      <div ref={issueListRef} className="flex-1 overflow-y-auto px-2 py-1">
        {(issuesLoading || (sessionSearchLoading && !!search.trim())) && issues.length === 0 ? (
          ...
          <div className="text-center py-8 text-[length:var(--fs-md)]" style={{ color: 'var(--text-muted)' }}>
          ...
          return (
            <div key={iss.id} className="mb-0.5">
              <Link to={`/u/${userParam}/p/${projectId}/i/${iss.id}`}
                ...
                <span className={`text-[length:var(--fs-md)] font-medium truncate flex-1 ${isCompleted ? 'line-through' : ''}`}
                  style={{ color: isCompleted ? 'var(--text-muted)' : 'var(--text-primary)' }}>
                  <SearchMatchText text={iss.title || ''} query={search} />
                </span>
                {iss.session_count > 0 && (
                  <span className="text-[length:var(--fs-xs)] px-1.5 py-0.5 rounded flex-shrink-0" style={{ color: 'var(--text-muted)', background: 'rgba(255,255,255,0.04)' }}>{iss.session_count}</span>
                )}
              </Link>
                ...
                return (
                  <Link key={session.session_id} to={`/u/${userParam}/p/${projectId}/i/${iss.id}?session=${session.session_id}`}
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-[length:var(--fs-sm)] font-medium" style={{ color: 'var(--text-primary)' }}>
              ...
              {sessionMatches.length > 3 && (
                <div className="ml-8 px-2 py-0.5 text-[length:var(--fs-xs)]" style={{ color: 'var(--text-muted)' }}>
      ...
      {showPagination && (
        <div className="px-3 py-2 border-t flex items-center justify-between gap-2" style={{ borderColor: 'var(--border-color)' }}>
          <button
            ...
            className="h-8 sm:h-7 px-2 rounded-md border text-[length:var(--fs-sm)] transition-colors hover:bg-[var(--bg-card-hover)] disabled:opacity-40 disabled:cursor-not-allowed"
            style={{ borderColor: 'var(--border-color)', color: 'var(--text-muted)' }}
          >
            上一页
          </button>
          <span className="text-[length:var(--fs-sm)] tabular-nums flex-shrink-0" style={{ color: 'var(--text-muted)' }}>
