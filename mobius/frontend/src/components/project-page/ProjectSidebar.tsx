    ...
    <>
      {/* <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: 'var(--border-color)' }}>
        ...
        <button onClick={onCreateIssue} disabled={!canCreateIssue} title={canCreateIssue ? '新建任务' : '无权新建任务'} data-tour="project-sidebar-new-issue"
          className="w-6 h-6 flex items-center justify-center rounded-md hover:bg-[var(--accent-soft)] text-[var(--accent-primary)] transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
      ...
      <div className="px-3 py-2 space-y-2">
        <div className="relative">
          ...
          <input value={search} onChange={e => onSearchChange(e.target.value)}
            ...
            placeholder="搜索任务或会话..."
            className="w-full h-8 pl-8 pr-8 rounded-lg text-[12px] focus:outline-none focus:border-[var(--accent-border)]"
          ...
          {(['active', 'all', 'completed'] as const).map(f => (
            <button key={f} onClick={() => onFilterChange(f)}
              className={`flex-1 h-7 rounded text-[11px] transition-colors ${
                filter === f ? 'bg-[var(--surface-active)] text-[var(--accent-primary)] border border-[var(--accent-border)]' : 'border border-transparent hover:bg-[var(--bg-card-hover)]'
              }`}
        ...
        {search.trim() && (sessionSearchError || sessionSearchTruncated) && (
          <div className="px-0.5 text-[10px] leading-4" style={{ color: sessionSearchError ? 'var(--status-danger)' : 'var(--text-muted)' }}>
        ...
        {(issuesLoading || (sessionSearchLoading && !!search.trim())) && issues.length === 0 ? (
          ...
          return (
            <div key={iss.id} className="mb-0.5">
              <Link to={issuePath(userParam, projectId, iss.id, { returnTo })}
                ...
                {!!iss.pinned && <svg className="w-2.5 h-2.5 flex-shrink-0" style={{ color: 'var(--accent-primary)' }} fill="currentColor" viewBox="0 0 24 24"><path d="M16 3l5 5-3 1-2 4-3 1-3-3-3 1-2-2 6-6-1-3 3-3-3-2 4-1z" /></svg>}
                <svg className="w-3.5 h-3.5 flex-shrink-0" style={{ color: isCompleted ? 'var(--status-success)' : 'var(--text-muted)' }} fill={isCompleted ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>
              ...
              {sessionMatches.slice(0, 3).map((session) => {
                ...
                return (
                  <Link key={session.session_id} to={sessionPath(userParam, session.session_id)}
                    data-project-session-match={session.session_id}
                    className="ml-6 flex min-h-8 items-center gap-2 rounded-md border-l-2 border-[var(--accent-border)] px-2 py-1 transition-colors hover:bg-[var(--bg-card-hover)]"
                    title={session.description || session.name}>
                    <MessageSquare className="h-3 w-3 flex-shrink-0 text-[var(--accent-primary)]" />
