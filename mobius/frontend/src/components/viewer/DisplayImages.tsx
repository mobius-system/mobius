    ...
    <figure className="m-0 flex flex-col gap-1">
      {err ? (
        <div className="flex flex-col items-center justify-center gap-1.5 h-44 rounded border border-dashed border-[var(--border-color)] bg-[var(--prose-bg)] px-3 text-center">
          <span className="text-[length:var(--fs-sm)] text-[var(--text-muted)]">图片加载失败</span>
          {reason && (
            <span className="text-[length:var(--fs-xs)] font-mono text-[var(--text-muted)] opacity-80 break-all line-clamp-3">
              {reason}
            </span>
          )}
          {isUrl && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-0.5 text-[length:var(--fs-sm)] text-teal-500 hover:underline">
              打开原图
            </a>
          )}
        </div>
      ...
      <figcaption className="text-[length:var(--fs-xs)] font-mono text-[var(--text-muted)] break-all select-text">
        {isUrl ? 'URL · ' : '本地 · '}{src}
      </figcaption>
    </figure>
  )
    ...
    <div
      ...
      <div
        ...
        <div className="flex items-center gap-3 border-b px-4 py-2.5" style={{ borderColor: 'var(--border-color)' }}>
          <span className="min-w-0 flex-1 truncate text-[length:var(--fs-md)] font-mono" style={{ color: 'var(--text-secondary)' }}>
            {srcLabel} · {srcDisplay}
          </span>
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="h-7 rounded-xl border px-2.5 py-1 text-[length:var(--fs-sm)] transition-colors hover:bg-[var(--bg-card-hover)]"
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex h-7 w-7 items-center justify-center rounded-xl border text-[length:var(--fs-2xl)] leading-none transition-colors hover:bg-[var(--bg-card-hover)]"
            style={{ color: 'var(--text-secondary)', borderColor: 'var(--border-color-strong)' }}
            aria-label="关闭放大图"
            title="关闭">
            ×
        ...
        <div className="flex min-h-0 flex-1 items-center justify-center overflow-auto bg-black/20 p-3">
          {err ? (
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              <span className="text-[length:var(--fs-md)] text-[var(--text-muted)]">图片加载失败 / 无法访问</span>
              <a
                href={openOriginalHref}
                target="_blank"
                rel="noreferrer"
                className="text-[length:var(--fs-md)] text-teal-500 hover:underline">
                打开原图
              </a>
            </div>
          ) : (
    ...
    <>
      <details
        ...
        <summary className={`cursor-pointer px-3 pt-1.5 ${open ? 'pb-0.5' : 'pb-1.5'} flex items-center gap-2 text-[length:var(--fs-md)] select-text`}>
          {typeof lineNo === 'number' && <span className="text-[length:var(--fs-xs)] text-[var(--text-muted)] font-mono flex-shrink-0">↳#{lineNo}</span>}
          ...
          <span className="text-[length:var(--fs-sm)] text-[var(--text-muted)] truncate flex-1">{sourceLabel} · {images.length} 张</span>
        </summary>
