...
// =====================================================================
// 上下文拼装: 文本块全部定义在 ./session-context-sections (前后端共用的零依赖目录,
// 标题与匹配正则同源派生), 这里只负责把服务端能力预计算注入 ctx, 再按顺序编排。
// =====================================================================

// 注入语言归一: 仅 'zh' / 'en', 其余回退中文.
...
// deps 仅供测试注入确定性桩, 生产走 REAL_FORMAT_DEPS。
function buildSectionCtx(sources: any, D: FormatDeps): SectionCtx {
  const research = sources.research;
  const session = sources.session;
  const realSessionId = session?.session_id && session.session_id !== '(待创建)' ? session.session_id : '';
  const wtApplies = !!(sources.issue && sources.issue.use_worktree && sources.project && sources.project.bind_path);
  return {
    ...sources,
    env: D.env,
    ...
    session_is_assistant: D.isAssistantSession(session),
  };
}

// 段间连接复刻旧版 lines.push 序列: 默认 '\n\n'; completionFlag.tightNext='\n'
// (旧版该段尾不带空行, 其后的 Issue/Session 紧跟); pcTaskMode.doubleGap='\n\n\n'
// (旧版该段以 '\n## ' 开头, 与前段间双空行)。
export function formatBody(sources: any, deps?: Partial<FormatDeps>): string {
  const D: FormatDeps = { ...REAL_FORMAT_DEPS, ...(deps || {}) };
  const lang = normalizeLanguage(sources.language);
  const ctx = buildSectionCtx(sources, D);
  let out = '';
  let prevTight = false;
  for (const section of SESSION_SECTIONS) {
    const text = section.build[lang](ctx, section.title);
    if (!text || !text.trim()) continue;
    if (!out) out = text;
    else out += (section.doubleGap ? '\n\n\n' : prevTight ? '\n' : '\n\n') + text;
    prevTight = !!section.tightNext;
  }
  return out.trimEnd();
}
...
function wrapUserMessage(body: string, userMessage: any, language: any): string {
  ...
  const heading = normalizeLanguage(language) === 'en' ? QuestionTitle.en : QuestionTitle.zh;
  return `${body}\n\n---\n\n${heading}\n${userMessage}`;
}
