...
## JSONL 追底「时而不工作」三因 (2026-09-22 诊断, 只查未改)

- 追底只有两条独立路径: ① `chat.tsx` 的 `EntriesAutoScroll` (条目数变化 + 内容根 ResizeObserver → lerp 追赶环); ② `Chat` 的 `[messages, streamContent, isTyping]` effect (直接 `scrollTop = scrollHeight`)。熄火开关三个: `userScrolledUpRef` / `searchHighlightActiveRef` / `matchTargetActiveRef`。
- 因A — vite dev (:45618) 必现, 生产产物 (:45616) 不受影响: `chat.tsx` 里 `useEffect(() => () => { if (rafRef.current) cancelAnimationFrame(rafRef.current) }, [])` 只 cancel, 不把 `rafRef.current` 归 0; `main.tsx:13` 开着 `React.StrictMode`, dev 下 mount→cleanup→mount, 挂载时 count effect 刚 schedule 的那帧被 cleanup 取消, 第二次 mount 的 `schedulePin()` 撞上残留非 0 的 `rafRef.current` (`if (!rafRef.current)` 恒假) → 条目驱动的追底**整会话死掉, 只能整页刷新**。证据: 给 rAF 打点, dev 有 2 个「已 schedule / 已 cancel / 从未运行」的帧, 栈为 `schedulePin (chat.tsx:154) ← chat.tsx:167 ← commitHookEffectListMount`; prod 同测 0 个。dev 全程 0 条 `[scroll] chase` 日志, prod 正常出 `chase: lerp` / `chase: done`。
