...
.EXAMPLE
  irm https://serve.nutshellai.cn/publish/auto/tutorial/deploy-windows-v6.ps1 | iex
#>
...
function Ensure-Repository([string] $Root) {
    ...
    if (-not (Test-Path (Join-Path $Root ".git"))) {
        ...
        Invoke-Checked "git" @("-c", "core.autocrlf=false", "clone", "https://floral-sun-8219.mobius-os.workers.dev/https://github.com/mobius-system/mobius.git", $Root)
    } else {
        Say "使用已有仓库: $Root"
        Push-Location $Root
        try {
...
function Ensure-ChinaDockerfiles([string] $Root) {
    $base = Join-Path $Root "deploy\onekey\Dockerfile4zh-base"
    $exe = Join-Path $Root "deploy\onekey\Dockerfile4zh-app"
...
function Print-BootstrapUsers([string] $Root) {
    ...
    foreach ($entry in ($line -split ';')) {
        ...
        $displayName = if ($parts.Count -ge 4) { $parts[3] } else { "" }
        Write-Host ("  Account:      {0}" -f $account)
        Write-Host ("  Password:     {0}" -f $password)
        Write-Host ("  Role:         {0}" -f $role)
        if ($displayName) { Write-Host ("  Display name: {0}" -f $displayName) }
        Write-Host ""
    }
}
...
try {
    Write-Host ""
    Say "Mobius Windows 一键部署（deploy-windows-v6）"
    Say "目录: $InstallDir"
    Wait-DockerReady
    ...
    try {
        ...
        $baseDockerfile = if ((Test-Path "deploy\onekey\Dockerfile4zh-base") -and (Test-Path "deploy\onekey\Dockerfile4zh-app")) { "deploy\onekey\Dockerfile4zh-base" } else { "deploy\Dockerfile" }
        $exeDockerfile = if ($baseDockerfile -eq "deploy\onekey\Dockerfile4zh-base") { "deploy\onekey\Dockerfile4zh-app" } else { "Dockerfile" }
        if ($baseDockerfile -eq "deploy\onekey\Dockerfile4zh-base") {
