...
.EXAMPLE
  irm https://serve.nutshellai.cn/publish/auto/tutorial/deploy-windows-v1.ps1 | iex
#>
...
function Wait-DockerReady {
    ...
    $info = $null
    try { $info = & docker info 2>$null } catch { $info = $null }
    if ($LASTEXITCODE -ne 0 -and (Test-Path "$env:ProgramFiles\Docker\Docker\Docker Desktop.exe")) {
    ...
    if ([string]::Join("`n", $info) -match "OSType:\s*windows") {
...
function Ensure-ChinaDockerfiles([string] $Root) {
    ...
    if ((Test-Path $base) -and (Test-Path $exe)) { return }
...
try {
    Write-Host ""
    Say "Mobius Windows 一键部署（deploy-windows-v1）"
    Say "目录: $InstallDir"
    Wait-DockerReady
