...
>
> Windows 一键安装（PowerShell 5.1+）：`irm https://serve.nutshellai.cn/publish/auto/tutorial/deploy-windows-v6.ps1 | iex`。运行前请安装并启动 Docker Desktop、Git for Windows 和 Python 3.10+；脚本会自动使用 `docker.1ms.run` 拉取基础镜像，并在首次克隆时禁用 Git 自动换行转换。
>
